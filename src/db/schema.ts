...
/**
 * OpenClaw skills catalog — installable capabilities for the OpenClaw agent platform.
 *
 * Separate from the main listings table because skills have unique fields
 * (installCmd, publisher, skillType) and a distinct data sourcing pipeline
 * (ClawHub.ai API, awesome-openclaw GitHub lists).
 *
 * Column conventions mirror the listings table:
 * - JSON arrays stored as TEXT (tags): parse at read time
 * - Booleans stored as INTEGER with { mode: 'boolean' }
 * - Timestamps stored as INTEGER with { mode: 'timestamp' } (Unix ms)
 *
 * Categories are defined in src/lib/categories.ts (SKILL_CATEGORIES).
 * All writes go through SkillService (src/services/skills.ts).
 */
export const skills = sqliteTable('skills', {
  /** UUID primary key — generated by crypto.randomUUID() in SkillService */
  id: text('id').primaryKey(),

  /** URL-safe slug for skill detail pages, e.g. "alpha-finder". Must be unique. */
  slug: text('slug').notNull().unique(),
  ...
  /** Skill category — one of 6 values defined in SKILL_CATEGORIES in categories.ts */
  category: text('category').notNull(),

  /** JSON array of normalized canonical tags, e.g. '["automation","web3"]' */
  tags: text('tags').notNull(),

  /** Canonical source URL (dedup key). Normalized before write. */
  sourceUrl: text('source_url').notNull().unique(),
  ...
  /** Skill publisher / author name or org, e.g. "getfoundry", "OpenClaw" */
  publisher: text('publisher'),

  /** Install command, e.g. "openclaw plugins install alpha-finder" */
  installCmd: text('install_cmd'),

  /** Skill type: 'agent-tool' | 'api-wrapper' | 'workflow' | 'data-source' | 'other' */
  ...
  /**
   * True if the sourceUrl returns 404 or 410.
   * Dead skills are excluded from browse results.
   */
  deadLink: integer('dead_link', { mode: 'boolean' }).default(false),
  ...
  /**
   * Hype score (0-100) computed from stars, recency, and upvotes.
   * 0 = not yet scored. Updated by compute-hype-scores script.
   */
  hypeScore: integer('hype_score').default(0),
});

/** Full skill record returned by SELECT queries */
export type Skill = typeof skills.$inferSelect;

/** Skill shape for INSERT operations (timestamps and defaults may be omitted) */
